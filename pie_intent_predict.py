...
class PIEPredict(object):
    ...
    def intent_speed_trajectory(self):
        """
        Generates the encoder decoder method
        :return: An instance of the network model
        """
        ...
        # Predict trajectory
        # Generate input data. the shapes is (sequence_lenght,length of flattened features)
        _traj_input = Input(shape=(self._observe_length, self._encoder_feature_size),
                               name='encoder_input')

        # Temporal attention module
        _attention_net = self.attention_temporal(_traj_input, self._observe_length)
